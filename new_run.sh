...
srun singularity run --nv --bind /scratch conda.sif python triton/$experiment_name/phc/train.py train.params.config.full_experiment_name="$experiment_name"  task=HumanoidAeMcpPnn6 task.env.numEnvs=512 headless=True test=False train.params.config.minibatch_size=8192 +debug1=False  +pythonpath=$experiment_name
