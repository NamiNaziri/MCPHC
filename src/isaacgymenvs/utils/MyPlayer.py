...
class MyPlayer(players.PpoPlayerContinuous):
    ...
    def run(self):
        ...
        self.experiment_dir = os.path.join(self.train_dir,  self.experiment_name + '_Eval')
        ...
        for _ in range(n_games):
            ...
            for n in range(self.max_steps):
                ...
                if(self.env.num_envs == 1):
                    ...
                    ref_anim_pos = self.env.vid_visualized_rb_body_pos[0].detach().cpu().numpy()

                    temp_rigid_body_pos = self.env._rigid_body_pos.clone().reshape(1,48,3)
                    temp_rigid_body_pos[...,:2] -=self.env.additive_agent_pos[...,:2]
                    temp_rigid_body_pos = temp_rigid_body_pos.reshape(1,2,-1,3)
                    temp_rigid_body_pos[:,1,...] += (self.env.additive_agent_pos.reshape(1,2,-1,3)[:,1,...] - self.env.additive_agent_pos.reshape(1,2,-1,3)[:,0,...])

                    temp_rigid_body_pos = temp_rigid_body_pos.reshape(1,48,3)[0].cpu().numpy()
                    
                    vd.body_markers[0].set_data(pos=temp_rigid_body_pos, size=0.1, edge_width=0.0, edge_color=(0, 0, 0, 0), face_color=(1, 0, 0, 1))

                    vd.body_markers[1].set_data(pos=np.array([[0, 0, 0]]),size=0.1,edge_width=0.01)
                    ...
                    vd.body_markers[1].set_data(pos=ref_anim_pos, size=0.1, edge_width=0.0, edge_color=(0, 0, 0, 0), face_color=(0, 0, 1, 1))
                ...
                if(self.env.num_envs == 1):
                    ...
                    anim_min_dist_list.append(torch.cdist(self.env.modified_rb_body_pos.reshape(1, 2, 24, 3)[:,0,...], self.env.modified_rb_body_pos.reshape(1, 2, 24, 3)[:,1,...]).min())
                    ...
                    if self.print_stats:
                        if print_game_res:
                            ...
                            if(self.env.num_envs == 1):
                                ...
                                #TODO uncomment this
                                self.writer.add_video('Video/v', nvid, self.last_checkpoint, dataformats="NTHWC", fps=25)
                                print('video added')
                                
                                plt.plot(self.steps_array, min_dist_list, color='red')  # Plot the values against the number of epochs
                                plt.plot(self.steps_array, anim_min_dist_list, color='blue')
                                plt.xlabel('steps')
                                plt.ylabel('Min distance')
                                plt.grid(True)
                                self.writer.add_figure('Test/Min distance rigid body', plt.gcf(), global_step=self.last_checkpoint)
        ...
        self.writer.add_scalar(f'Test/Average Reward', sum_rewards / games_played * n_game_life, self.last_checkpoint)
        self.writer.add_scalar(f'Test/Average Steps', sum_steps / games_played * n_game_life, self.last_checkpoint)
    ...
    def update_history_lists(self):
        ...
        self.ref_anim_history.append(self.env.modified_rb_body_pos.reshape(1, 2, 24, 3).clone())
    ...
    def get_contact_force_from_history(self,agent, body_name):
        ...
        return [torch.norm(tensor[:,agent,self.env._body_names.index(body_name),:]) for tensor in self.contact_forces_history]

    def plot_cumulative_contact_forces(self,body_name):
        self.writer.add_scalar(f'ComulativeContactForce/{body_name}', torch.stack(self.get_contact_force_from_history(1,body_name)).sum(), self.last_checkpoint)

    def plot_contact_forces(self, body_name):
        plt.plot(self.steps_array, self.get_contact_force_from_history(0,body_name), color='blue') 
        plt.plot(self.steps_array, self.get_contact_force_from_history(1,body_name), color='red')  # Plot the values against the number of epochs
        plt.xlabel('steps')
        plt.ylabel('force magnitude')
        plt.grid(True)
        self.writer.add_figure(f'ContactForce/{body_name} force magnitude', plt.gcf(), global_step=self.last_checkpoint)

    def plot_dist(self, body_name):
        plt.plot(self.steps_array, self.get_distance_from_history(body_name, self.rigid_body_history), color='red')  # Plot the values against the number of epochs
        plt.plot(self.steps_array, self.get_distance_from_history(body_name, self.ref_anim_history), color='blue')         
        plt.xlabel('steps')
        plt.ylabel('Min distance')
        plt.grid(True)
        self.writer.add_figure(f'Distance/{body_name} min distance', plt.gcf(), global_step=self.last_checkpoint)
