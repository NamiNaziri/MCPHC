...
class MotionLibBase():
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1):
        ...
        #TODO correct this.num_motion_to_load = len(skeleton_trees)
        ...
        self.lts = torch.cat([m.local_translation for m in motions], dim=0).float().to(self._device)
        self.ltf = torch.cat([m.local_transformation for m in motions], dim=0).float().to(self._device)
        file_path = 'torchready_all.pkl'

        import pickle
        # Save the dictionary to the pickle file
        with open(file_path, 'wb') as file:
            pickle.dump(self.gts, file)
        if flags.real_traj:
    ...
    def get_global_translation_from_local(self, local_translation, local_transformation):
