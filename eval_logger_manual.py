...
def monitor_directory(parent_directory, polling_interval=2):
   ...
   # print(previous_state.keys())
    ...
    for run in runs:
        for (directory, files) in previous_state.items():
            if( run in directory):
                eval_state[directory] = previous_state[directory]

    for directory, files in eval_state.items():
        for file in files:
            if ".pth" in file:
                ...
                print(f"New file created in {directory}: {os.path.join(directory, file)}")
    #print(eval_state)
    #{os.path.join(directory, file): os.path.getmtime(os.path.join(directory, file)) for (directory, files) in previous_state.items() for file in files}
